...
public class newMesh extends DotGen{
    ArrayList<Polygon> polygons = new ArrayList<>();
        public newMesh(){
            ...
            List<List<Integer>> listOfNeighbors = new ArrayList<>();
            List<Integer> neighborsIdx = new ArrayList<>();
            neighborsIdx.add(0);
            neighborsIdx.add(1);
            neighborsIdx.add(Integer.valueOf((int) (width/square_size - 1)));
            neighborsIdx.add(Integer.valueOf((int) (width/square_size)));
            listOfNeighbors.add(neighborsIdx);
            neighborsIdx.clear();

            for (int neighborCount = 1; neighborCount < (width/square_size - 1); neighborCount++){
                neighborsIdx.add(neighborCount - 1);
                neighborsIdx.add(neighborCount + 1);
                neighborsIdx.add(neighborCount + Integer.valueOf((int) (width/square_size - 2)));
                neighborsIdx.add(neighborCount + Integer.valueOf((int) (width/square_size - 1)));
                neighborsIdx.add(neighborCount + Integer.valueOf((int) (width/square_size)));
                listOfNeighbors.add(neighborsIdx);
                neighborsIdx.clear();
            }
            ...
            for (int rows = 0; rows < 23; rows++) {
                if (counter % 2 == 0) {
                    for (int i = counter * 49; i < ((counter + 1) * 49); i += 2) {
                        if (i == (46+(49*counter))){
                            Polygon p = Polygon.newBuilder().addSegmentIdxs(i).addSegmentIdxs(i+2).addSegmentIdxs(i+49).addSegmentIdxs(i+1).setCentroidIdx(centroidCount).build();
                            polygons.add(p);
                            centroidCount++;
                        }else if(i == (48+(49*counter))){
                            continue;
                        }else {
                            Polygon p = Polygon.newBuilder().addSegmentIdxs(i).addSegmentIdxs(i + 3).addSegmentIdxs(i + 49).addSegmentIdxs(i + 1).setCentroidIdx(centroidCount).build();
                            polygons.add(p);
                            centroidCount++;
                        }
                    }
            ...
            for (int x = 1127; x < 1173; x += 2){
                Polygon p = Polygon.newBuilder().addSegmentIdxs(x).addSegmentIdxs(x + 3).addSegmentIdxs(x + 49 - otherCounter).addSegmentIdxs(x + 1).setCentroidIdx(otherCounter).build();
                polygons.add(p);
                otherCounter++;
            }

            Polygon p = Polygon.newBuilder().addSegmentIdxs(1173).addSegmentIdxs(1175).addSegmentIdxs(1199).addSegmentIdxs(1174).setCentroidIdx(575).build();
            polygons.add(p);
