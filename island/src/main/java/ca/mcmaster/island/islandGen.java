...
public class islandGen {
    public Structs.Mesh lagoon(Structs.Mesh m){
        ...
        for(Structs.Polygon p : m.getPolygonsList()){
            ...
            if(d <= radius){
                tilePolygons.add(Structs.Polygon.newBuilder(p).addProperties(land.getColour()).build());
            }else{
                tilePolygons.add(Structs.Polygon.newBuilder(p).addProperties(water.getColour()).build());
            }
        }
